...
{
    int flag = 0;
    char aux_1[50], aux_2[50], aux_3[50], aux_4[50], aux_5[50];
    ...
    while(!feof(pFile))     //Mientras no se llege al final del archivo
    {
        ...
        if(status == 5)     //Si status es igual a los 4 datos pedidos a leer
        {
            //Le asigno a "this" las variables leidas casteadas en la funcion

            eEditorial* this = _setters(aux_1, aux_2, aux_3, aux_4, aux_5);
...
int parser_FromBinary(FILE* pFile, LinkedList* pArrayList)
{
    ...
    while(!feof(pFile))
    {
        ...
        int status = fread(this, sizeof(eEditorial), 1, pFile);
