...
static Node* getNode(LinkedList* this, int nodeIndex)
{
    ...
    {
        pNode = this->pFirstNode;   //Asigno la direccion de memoria del primer nodo de la LinkedList a pNode
        for (int i=0; i<nodeIndex; i++)
            pNode = pNode->pNextNode; //Asigno la direccion de memoria del siguiente nodo
    }
    return pNode;
}
...
void* ll_get(LinkedList* this, int index)
{
    ...
    if (this != NULL && getNode(this, index) != NULL) //Entra si this y la direccion de memoria que retorne getNode son distintas a NULL
        pElement = (getNode(this, index))->pElement;   //Asigno a pElement el puntero del elemento que tiene el nodo
        ...
        if (filterList != NULL)
        {
            for (int i=0; i<ll_len(this); i++)
            {
                if (pFunc(ll_get(this, i)))                  //Condicional de la funcion a escoger
                    ll_add(filterList, ll_get(this, i));    //Asigna el nodo a la nueva lista
            }
        }
    }
    return filterList;
