...
{
    void* pNode = NULL; //Puntero a nodo inicializado en NULL
    if (this != NULL && getNode(this, index) != NULL) //Entra si this y la direccion de memoria que retorne getNode son distintas a NULL
        pNode = (getNode(this, index))->pElement;   //Asigno a pNode el valor del elemento que tiene el nodo
...
{
    int returnAux = -1;
    if (this != NULL && getNode(this, index) != NULL)
    {
        if (!index) //Entra si el indice es igual a 0
            this->pFirstNode = NULL;
        else //Busco el puntero que contiene el puntero a eliminar, es decir el nodo anterior y le asigno el siguiente nodo del puntero a eliminar
...
int ll_indexOf(LinkedList* this, void* pElement)
{
    ...
    if (this != NULL)
    {
        for(int i=0; i<ll_len(this); i++)
        {
            ...
            {
                returnAux = i;  //asigna a returnAux el valor de la posicion del elemento
                break;
            }
        }
    }
...
{
    Node* pNode = ll_get(this, index);  //Obtiene el puntero pElement
...
int ll_contains(LinkedList* this, void* pElement)
{
    ...
    if (this != NULL)
    {
        if (ll_indexOf(this, pElement) == -1)     //Condicional, si la funcion retorna (-1)
            flag = 0;
        else
            flag = 1;
    }
...
int ll_containsAll(LinkedList* this, LinkedList* this2)
{
    ...
    if (this != NULL && this2 != NULL)
    {
        ...
        for (int i=0; i<ll_len(this); i++)
        {
            ...
            if(flag == -1)
                break;  //Rompo bucle si flag es (-1)
            else if(!flag)
                break;  //Rompo bucle si flag es (0)
            else
                continue;
        }
    }
...
{
    LinkedList* clone = NULL; //asigno al puntero clone la no direccion de memoria
    if (this != NULL)
    {
        if (from >= 0 && to <= ll_len(this)) //Verifico que to y from esten dentro del size del LinkedList
        {
            clone = ll_newLinkedList();    //le asigno una nueva lista a clone
            if (clone != NULL)
            {
                for (int i=from; i<to; i++)
                    ll_add(clone, ll_get(this, i));//Le paso como parametro el valor del elemento a ll_add y lo enlaza con los demas nodos
            }
        }
    }
    return clone;
...
int ll_sort(LinkedList* this, int (*pFunc)(void*,void*), int order)
{
    ...
    if(this != NULL && pFunc != NULL && (order == 1 || order == 0))
    {
        ...
        {
            Node* pNodeUno = ll_get(this, i);     //Asigno a pNodeUno la direccion que retorna ll_get()
            for(int j=i+1; j<ll_len(this); j++)
            {
                Node* pNodeDos = ll_get(this, j); //Asigno a pNodeDos la direccion que retorna ll_get()
                if(order)
                {
                    if(pFunc(pNodeUno, pNodeDos) > 0) //Aplico Burbujeo si la funcion llamada es mayor a cero
                    {
                        Node* pNodeAux = pNodeDos;
                        pNodeDos = pNodeUno;
                        pNodeUno = pNodeAux;
                    }
                }
 ...
 *
 */
LinkedList* ll_map(LinkedList* this, int (*pFunc)(void*,char*))
{
    LinkedList* filterList;
    Dominio* aux = dominio_new();
    char tipo;
    if (this != NULL)
    {
        ...
        if (filterList != NULL)
        {
            ...
            {
                aux = (Dominio*)ll_get(this, i);
                if (pFunc(aux, &tipo) == 0)    /**< Compruebo que el tipo fue calculado */
                {
                    if (dominio_setTipo(aux, tipo))
